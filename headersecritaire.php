...
$req="SELECT * from user where id_user='".$_SESSION["id"]."' and role='secritaire'" ;
$res=$conx->query($req);
if(!$res)
{
...
<body>
    <div class=" heder1 header">
        <div style="margin-right:12px;" class="d-flex justify-content-between">
            <div>
           <a href="dashboardsecitaire.php"> <img class="image-stage "  src="img/logo2.png"></a>
            </div>
            
            
            <div style="padding-top:5px;">
                <label style="color:white;margin-top:px;font-size: 25px;"><?php echo "$row[nom] $row[prenom]"?></label>
                <a href='imagesecritaire.php?code_u=<?= $row['id_user'];?>'> <img class="image-profil rounded-circle"  src=" <?= $row['image'];?>"></a>
