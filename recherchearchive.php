...
$input=$_POST['input'];
$res=mysqli_query($conx,"SELECT * from affectation a join demande d on(d.id_demande=a.id_demande)
 join stagiare s on(d.id_stagiare=s.id_stagiare)
  where (nom like '{$input}%' OR prenom like '{$input}%') and( a.status='supprimer')limit 6");
